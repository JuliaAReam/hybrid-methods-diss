...
\end{aligned}
\end{equation}
$G$ is the convolution kernel of associated filter spatial and/or temporal cutoff $\Delta$ and $\tau_c$, respectively, satisfying $\int\limits_{-\infty}^{\infty}\int\limits_{-\infty}^{\infty}G(\vb{r}, s) d\vb{r}ds$. In practice, common kernels are only spatially dependent. Applying this filtering procedure to the Navier-Stokes equations yields an additional term known as the residual stress tensor that must be modeled. The filtered Navier-Stokes equations can then be discretized to numerically simulated the large eddy behavior of the system.
